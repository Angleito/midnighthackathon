...
export const useGameState = create<GameStore>((set, get) => ({
  ...
  startBattle: async () => {
    // Simulate committing hidden stats (method not implemented yet)
    console.log('Starting battle with ZK commitment...');
    // Create a proper base64 commitment for the initial battle state
    const initialCommitment = btoa(`battle:start:${Date.now()}`);
    set({ phase: 'battle', playerCommitment: initialCommitment, turn: 1 });
  },
  ...
  commitPlayerMove: async (moveId: string) => {
    ...
    // Only log in game debug mode to reduce console spam
    if (import.meta.env.VITE_GAME_DEBUG === 'true') {
      console.log('Committing move to ZK proof system:', commitment);
    }
  ...
  resolveTurn: async () => {
    ...
    let moveId = '';
    let playerMove = player.moves[0]; // Default move
    ...
    try {
      ...
      if (parts.length >= 3) {
        moveId = parts[2];
        const foundMove = player.moves.find(m => m.id === moveId);
        if (foundMove) {
          playerMove = foundMove;
        }
      }
    } catch (error) {
      console.warn('Failed to decode player commitment, using default move:', error);
      // Use default move (already set above)
    }
    ...
    // Placeholder ZK verification - only log in debug mode
    if (import.meta.env.VITE_GAME_DEBUG === 'true') {
      console.log('Verifying ZK proof for moves:', [playerMove.name, enemyMove.name]);
    }
